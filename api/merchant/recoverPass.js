    ...
    try {
        ...
        if (step == 'FIRST') {
            ...
            if (hashedToken) {
                ...
                // store in database
                const result = await MerchantModel.findOneAndUpdate({ email }, { testToken: hashedToken }, { new: true })
        ...
        if (step == 'SECOND') {
            ...
            // store in database
            await MerchantModel.findOneAndUpdate({ email }, { testToken: 'NULL' }, { new: true })
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        const user = MerchantModel.findOne({ _id: req.users._id })
        ...
        const update = await MerchantModel.findOneAndUpdate({ _id: req.users._id }, { password: hashedPassword }, { new: true })
