    ...
    try {
        ...
        if (step == 'FIRST') {
            ...
            if (hashedToken) {
                ...
                // store in database
                const result = await MerchantModel.findOneAndUpdate({ email }, { recoveryToken: hashedToken }, { new: true })
        ...
        if (step == 'SECOND') {
            ...
            // store in database
            await MerchantModel.findOneAndUpdate({ email }, { recoveryToken: 'NULL' }, { new: true })
        ...
        if (step == 'THIRD') {
            ...
            const result = await MerchantModel.findOneAndUpdate({ email }, { password: hashedPassword }, { new: true })
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        const user = MerchantModel.findOne({ _id: req.user._id })
        ...
        const update = await MerchantModel.findOneAndUpdate({ _id: req.user._id }, { password: hashedPassword }, { new: true })
